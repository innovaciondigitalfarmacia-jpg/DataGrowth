...
const Factory = ({ brands, gemKey }) => {
  ...
  const go = async () => {
    ...
    const fmt = ct.fmt;
    let msg = fmt==="visual"?`Post:"${topic}".JSON:{"headline":"max 8 palabras","subtext":"subtitulo","caption":"3-5 lineas con emojis","hashtags":"8 hashtags","image_prompt":"english: write a detailed specific prompt describing an image about this exact topic for the brand. Describe objects, people, scenes to show. Brand style: professional modern. High quality social media graphic. IMPORTANT: absolutely no text, no words, no letters in the image"}`:fmt==="carousel"?`Carrusel 5:"${topic}".JSON:{"slides":[{"title":"..","body":"emojis","emoji":".."}],"caption":"CTA","hashtags":"8"}`:fmt==="reel"?`Reel 5 escenas:"${topic}".JSON:{"scenes":[{"title":"..","duration":"3s","emoji":"..","visual":"cámara","text_overlay":"texto+emoji","audio":"música","transition":"tipo"}],"caption":"CTA","hashtags":"8"}`:`${ct.label}:"${topic}".Emojis,Gancho→Valor→CTA.8 hashtags.`;
