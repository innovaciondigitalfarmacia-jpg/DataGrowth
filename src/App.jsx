    ...
    <Section title="Crear Contenido" sub="Genera contenido profesional con IA." right={<Badge>Claude AI</Badge>}>
      ...
      {ct.fmt==="visual"&&chatHistory.length>0&&(() => { const last = [...chatHistory].reverse().find(m => m.role === "ai"); if (!last) return null; return <div style={{marginBottom:16}}>
          <Card>
            ...
            {last.img&&<div style={{marginBottom:14}}><div style={{borderRadius:14,overflow:"hidden",position:"relative"}}><img id="ai-latest-img" crossOrigin="anonymous" src={last.img} alt="AI" onContextMenu={e=>e.preventDefault()} onDragStart={e=>e.preventDefault()} style={{width:"100%",maxHeight:500,objectFit:"contain",display:"block",borderRadius:14,WebkitUserSelect:"none",userSelect:"none"}}/><div onContextMenu={e=>e.preventDefault()} style={{position:"absolute",top:0,left:0,right:0,bottom:0,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}><span style={{fontSize:38,fontWeight:900,color:"rgba(255,255,255,0.3)",textShadow:"0 2px 8px rgba(0,0,0,0.3)",letterSpacing:6,textTransform:"uppercase",transform:"rotate(-25deg)",userSelect:"none",whiteSpace:"nowrap"}}>PREVIEW</span></div></div><div style={{textAlign:"center",padding:"6px 0",fontSize:11,color:t.txM}}>⬇️ Descarga la imagen para obtenerla sin marca de agua</div></div>}
      ...
      {result&&!loading&&result.t==="reel"&&result.d?.scenes&&<Card>
        {videoUrl&&<div style={{marginBottom:16}}><div style={{borderRadius:14,overflow:"hidden",position:"relative"}}><video src={videoUrl} controls controlsList="nodownload" onContextMenu={(e)=>e.preventDefault()} style={{width:"100%",maxHeight:400,borderRadius:14,display:"block"}}/><div style={{position:"absolute",top:0,left:0,right:0,bottom:40,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}><span style={{fontSize:30,fontWeight:900,color:"rgba(255,255,255,0.25)",textShadow:"0 2px 8px rgba(0,0,0,0.3)",letterSpacing:6,textTransform:"uppercase",transform:"rotate(-25deg)",userSelect:"none",whiteSpace:"nowrap"}}>PREVIEW</span></div></div><div style={{textAlign:"center",padding:"6px 0",fontSize:11,color:t.txM}}>⬇️ Descarga el video para obtenerlo sin marca de agua</div></div>}
