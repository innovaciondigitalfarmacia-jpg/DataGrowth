...
const Factory = ({ brands, gemKey }) => {
  ...
  const go = async () => {
    ...
    if (brand.website) {
      try {
        const wr = await fetch("/api/scrape?url=" + encodeURIComponent(brand.website));
  ...
  return (
    <Section title="Crear Contenido" sub="Genera contenido profesional con IA." right={<Badge>Claude AI</Badge>}>
      ...
      <div style={{marginBottom:14}}><Label>Tipo</Label><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>{CTYPES.map(c=><button key={c.id} onClick={()=>{setCt(c);if(c.fmt!=="reel"){setReelImage(null);setReelImagePreview(null);}}} style={{padding:"12px 10px",borderRadius:12,border:ct.id===c.id?`2px solid ${t.ac}`:`1px solid ${t.brd}`,background:ct.id===c.id?t.acS:t.bgC,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:22,marginBottom:4}}>{c.icon}</div><div style={{fontSize:12,fontWeight:600,color:ct.id===c.id?t.tx:t.txS}}>{c.label}</div></button>)}</div></div>
      {ct.fmt==="reel"&&<div style={{marginBottom:14,padding:14,background:t.bgI,borderRadius:12,border:"1px solid "+t.brd}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:14}}>📷</span><span style={{fontSize:12,fontWeight:600,color:t.tx}}>Foto de referencia</span><span style={{fontSize:11,color:t.txM,fontStyle:"italic"}}>(opcional)</span></div><div style={{fontSize:11,color:t.txS,marginBottom:10,lineHeight:1.5}}>Si subes una foto real de tu producto o lugar, el video se genera a partir de ella y se ve mas realista. Si no subes foto, la IA crea el video desde cero.</div><div style={{display:"flex",gap:12,alignItems:"center"}}><label style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",border:"2px dashed "+(reelImage?t.ac:t.brd),borderRadius:10,cursor:"pointer",color:reelImage?t.ac:t.txM,fontSize:12,fontWeight:500,background:reelImage?t.acS:"transparent"}}><span style={{fontSize:16}}>📷</span>{reelImage?"Cambiar foto":"Subir foto"}<input type="file" accept="image/*" onChange={handleReelImage} style={{display:"none"}}/></label>{reelImagePreview&&<div style={{position:"relative"}}><img src={reelImagePreview} style={{width:50,height:50,borderRadius:8,objectFit:"cover",border:"2px solid "+t.ac}}/><div onClick={()=>{setReelImage(null);setReelImagePreview(null);}} style={{position:"absolute",top:-6,right:-6,width:16,height:16,borderRadius:"50%",background:"#ef4444",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,cursor:"pointer",fontWeight:700}}>x</div></div>}{reelImage&&<span style={{fontSize:11,color:t.ac,fontWeight:600}}>✅ El video se animara desde esta foto</span>}</div></div>}
