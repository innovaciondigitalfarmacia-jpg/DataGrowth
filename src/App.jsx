...
const Factory = ({ brands, gemKey }) => {
  ...
  const go = async () => {
    ...
    const sys = "Eres DIRECTOR CREATIVO SENIOR agencia Bogota. " + brandCtx + ". REGLAS:1)Gancho 2)3-5 emojis 3)Gancho>Valor>CTA 4)8 hashtags 5)Espanol colombiano tu 6)Valor primero 7)Saltos linea. SOLO JSON sin markdown sin backticks.";
    const fmt = ct.fmt;
    const imgInst = "image_prompt: MUST be in english. MUST describe EXACTLY what the user asked for. Include these brand colors: " + brandColors + ". Brand visual style: " + brandStyle + ". Industry: " + brand.industry + ". Be VERY specific and literal about the scene, objects, people. NEVER be generic. NO text, NO words, NO letters in image.";
    let msg = "";
    if (fmt === "visual") {
      msg = 'Post sobre: "' + topic + '". Responde SOLO con JSON asi: {"headline":"max 8 palabras","subtext":"subtitulo","caption":"3-5 lineas con emojis","hashtags":"8 hashtags","image_prompt":"(en ingles) descripcion MUY especifica y literal de EXACTAMENTE lo que el usuario pidio. ' + imgInst + '"}';
