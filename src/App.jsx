          ...
          {step === 5 && <>
            ...
            <div style={{ padding: 14, background: t.bgI, borderRadius: 12, border: "1px solid " + t.brd, marginBottom: 14 }}>
              ...
              :
                <button onClick={async () => {
                  if (!f.id) {
                    // Save brand first to get ID, then connect
                  ...
                  const handler = async (e) => { if (e.data?.type === "ig_connected") { window.removeEventListener("message", handler); const token = e.data.ig_token || ""; const igUserId = e.data.ig_user_id || ""; const igUsername = "@" + (e.data.username || ""); console.log("IG connected:", { token: token ? "yes(" + token.length + ")" : "no", igUserId, igUsername }); if (token && igUserId && f.id) { try { await supabase.from("brands").update({ ig_token: token, ig_user_id: igUserId, instagram: igUsername }).eq("id", f.id); u("ig_token", token); u("ig_user_id", igUserId); u("instagram", igUsername); } catch (err) { console.error("Save error:", err); } } else if (f.id) { const { data: check } = await supabase.from("brands").select("ig_token,ig_user_id,instagram").eq("id", f.id).single(); if (check?.ig_token) { u("ig_token", check.ig_token); u("ig_user_id", check.ig_user_id); u("instagram", check.instagram); } else { u("ig_user_id", igUserId || "connected"); u("instagram", igUsername); } } alert("✅ Instagram conectado: " + igUsername); } };
