            ...
            <div style={{ padding: 14, background: t.bgI, borderRadius: 12, border: "1px solid " + t.brd, marginBottom: 14 }}>
              ...
              :
                <button onClick={async () => {
                  ...
                  const handler = async (e) => { if (e.data?.type === "ig_connected") { window.removeEventListener("message", handler); const token = e.data.ig_token || ""; const igUserId = e.data.ig_user_id || ""; const igUsername = "@" + (e.data.username || ""); if (token && igUserId && f.id) { await supabase.from("brands").update({ ig_token: token, ig_user_id: igUserId, instagram: igUsername }).eq("id", f.id); u("ig_token", token); u("ig_user_id", igUserId); u("instagram", igUsername); } else { u("ig_user_id", "connected"); u("instagram", igUsername); } alert("✅ Instagram conectado: " + igUsername); } };
