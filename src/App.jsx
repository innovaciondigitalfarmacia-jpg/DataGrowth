    ...
    const draw = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        if (p.life > 0.75) {
          ...
          grad.addColorStop(0.3, `rgba(120,190,255,${alpha * 0.9})`);
          grad.addColorStop(1, `rgba(59,130,246,0)`);
        } else if (p.life > 0.45) {
          grad.addColorStop(0, `rgba(255,240,120,${alpha})`);
          grad.addColorStop(0.35, `rgba(255,170,40,${alpha * 0.8})`);
          grad.addColorStop(1, `rgba(249,115,22,0)`);
        } else if (p.life > 0.2) {
          grad.addColorStop(0, `rgba(239,80,50,${alpha * 0.7})`);
          grad.addColorStop(0.5, `rgba(180,40,20,${alpha * 0.4})`);
          grad.addColorStop(1, `rgba(80,20,10,0)`);
        } else {
          grad.addColorStop(0, `rgba(80,80,80,${alpha * 0.3})`);
    ...
    <div style={{ background: t.bg, minHeight: "100vh", fontFamily: "'Segoe UI',system-ui,sans-serif", color: t.tx }}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}} @keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}} @keyframes glow{0%,100%{text-shadow:0 0 20px rgba(55,194,235,0.3)}50%{text-shadow:0 0 40px rgba(55,194,235,0.6),0 0 80px rgba(55,194,235,0.2)}} .dg-title{${dark ? 'animation:glow 3s ease-in-out infinite' : ''}}
        @media(max-width:768px){
          .dg-grid3{grid-template-columns:1fr!important}
          .dg-grid4{grid-template-columns:repeat(2,1fr)!important}
          .dg-grid2{grid-template-columns:1fr!important}
          .dg-nav-links{display:none!important}
          .dg-nav-btns{display:none!important}
          .dg-nav-hamburger{display:flex!important}
          .dg-footer-links{flex-direction:column!important;gap:10px!important;align-items:flex-start!important}
          .dg-footer-bottom{flex-direction:column!important;text-align:center}
        }
        @media(min-width:769px){
          .dg-nav-hamburger{display:none!important}
        }
      `}</style>
      ...
      <nav style={{ padding: "0", borderBottom: `1px solid ${t.brd}`, position: "sticky", top: 0, background: t.bg + "f2", backdropFilter: "blur(16px)", zIndex: 50, height: 62, display: "flex", alignItems: "center" }}>
        <div style={{ maxWidth: 1100, width: "100%", margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="dg-nav-links" style={{ display: "flex", alignItems: "center", gap: 2 }}>
            {NAV_LINKS.map((l, i) => (
          ...
          <div className="dg-nav-btns" style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {/* Mobile menu */}
      <div id="dg-mobile-menu" style={{ display: "none", flexDirection: "column", background: t.bg, borderBottom: "1px solid " + t.brd, padding: "8px 24px 16px", position: "sticky", top: 62, zIndex: 49 }}>
        {NAV_LINKS.map((l, i) => <button key={i} onClick={() => { l.action(); const m = document.getElementById("dg-mobile-menu"); if(m) m.style.display = "none"; }} style={{ background: "transparent", border: "none", color: t.txS, fontSize: 14, fontWeight: 500, padding: "12px 0", cursor: "pointer", textAlign: "left", borderBottom: "1px solid " + t.brd }}>{l.label}</button>)}
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <Btn secondary onClick={() => { onLogin(); const m = document.getElementById("dg-mobile-menu"); if(m) m.style.display = "none"; }} style={{ fontSize: 13, padding: "10px 16px", flex: 1 }}>Iniciar sesion</Btn>
          <Btn primary onClick={() => { onRegister(); const m = document.getElementById("dg-mobile-menu"); if(m) m.style.display = "none"; }} style={{ fontSize: 13, padding: "10px 16px", flex: 1 }}>Crear cuenta</Btn>
        </div>
      </div>
      ...
      {(landingPage==="home"||landingPage==="funciones")&&<div id="funciones" style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 24px" }}>
        ...
        <div className="dg-grid3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {features.map((f, i) => (
      ...
      {(landingPage==="home"||landingPage==="quienes")&&<div id="quienes" style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 24px", position: "relative" }}>
        ...
        <div className="dg-grid2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          {[
        ...
        <div style={{ marginTop: 24, background: t.bgC, border: "1px solid " + t.brd, borderRadius: 16, padding: 28 }}>
          <div style={{ fontSize: 17, fontWeight: 700, color: t.tx, marginBottom: 16, textAlign: "center" }}>🛠️ Tecnologia que impulsa DataGrowth</div>
            <div className="dg-grid4" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
              {[
      ...
      {(landingPage==="home"||landingPage==="blog")&&<div id="blog" style={{ maxWidth: 1100, margin: "0 auto", padding: "80px 24px" }}>
        {blogOpen ? (
          ...
          <>
            ...
            <div className="dg-grid3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
      ...
      <div style={{ borderTop: `1px solid ${t.brd}`, padding: "40px 0 24px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20, marginBottom: 24 }}>
            ...
            <div className="dg-footer-links" style={{ display: "flex", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
              <a href="https://instagram.com/datagrowth.agency" target="_blank" rel="noopener noreferrer" style={{ color: t.txS, textDecoration: "none", display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 500, transition: "color .2s" }} onMouseEnter={e => e.currentTarget.style.color = "#E1306C"} onMouseLeave={e => e.currentTarget.style.color = t.txS}>
          ...
          <div className="dg-footer-bottom" style={{ borderTop: `1px solid ${t.brd}`, paddingTop: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
      ...
      {showPlans && <div style={{ position: "fixed", inset: 0, background: t.bg, zIndex: 200, overflow: "auto" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "60px 24px 80px" }}>
          ...
          <div className="dg-grid3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
            {PLANS.map(p => (
          ...
          {step === 5 && <>
            ...
            <div style={{ padding: 14, background: t.bgI, borderRadius: 12, border: "1px solid " + t.brd, marginBottom: 14 }}>
              ...
              :
                <button onClick={async () => {
                  if (!f.id) {
                    if (!f.name || !f.short) { alert("Completa el nombre y abreviatura de la marca primero"); return; }
                    alert("Primero guarda la marca (botón 'Crear marca' abajo), luego edítala para conectar Instagram.");
                    return;
                  }
                  ...
                  const handler = async (e) => {
                    if (e.data?.type === "ig_connected") {
                      ...
                      const token = e.data.ig_token || "";
                      const igUserId = e.data.ig_user_id || "";
                  ...
                  // Also poll for popup close, then reload brand from Supabase
...
const Factory = ({ brands, gemKey, isAdmin, user }) => {
  ...
  useEffect(() => { if (brand && brands.length) { const updated = brands.find(b => b.id === brand.id); if (updated) setBrand(updated); } else if (brands.length) { setBrand(brands[0]); } }, [brands]);
  ...
  const go = async () => {
    ...
    try { const gc = new AbortController(); const gt = setTimeout(() => gc.abort(), 60000); const r = await fetch("/api/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"gemini-2.5-flash",max_tokens:2500,system:sys,messages:[{role:"user",content:msg}],brand:brandFull}),signal:gc.signal}); clearTimeout(gt); const d = await r.json(); const raw = d.content?.map(c=>c.text||"").join("")||""; if(fmt==="text"){setTxt(raw);setResult({t:"text"});}else{try{let clean=raw;if(clean.indexOf("{")>-1)clean=clean.substring(clean.indexOf("{"),clean.lastIndexOf("}")+1);const pd=JSON.parse(clean);
      ...
      if(fmt==="visual"&&pd.image_prompt){
        if (!lastImageContext) setLastImageContext(currentTopic + " | " + pd.image_prompt.substring(0, 200));
      ...
      {ct.fmt==="visual"&&chatHistory.length>0&&(() => { const last = [...chatHistory].reverse().find(m => m.role === "ai"); if (!last) return null; return <div style={{marginBottom:16}}>
          <Card>
            ...
            {(last.img||last.text)&&!last.loading&&<div style={{marginTop:12,display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              {last.img&&<button disabled={publishing} onClick={async()=>{if(!brand?.ig_token||!brand?.ig_user_id){alert("⚠️ Instagram no conectado. Ve a Branding Kit → edita tu marca → Paso 5 → Conectar Instagram");return;}if(publishing)return;if(!isAdmin&&getLeft(ct.id)<=0){alert("Has alcanzado el limite de "+ct.label+" de tu plan. Actualiza para publicar mas.");return;}setPublishing(true);try{const caption=(last.text||"")+(last.hashtags?"\n\n"+last.hashtags:"");const r=await fetch(last.img);const b=await r.blob();const reader=new FileReader();reader.onloadend=async()=>{const b64=reader.result.split(",")[1];const res=await fetch("/api/instagram",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ig_token:brand.ig_token,ig_user_id:brand.ig_user_id,image_base64:b64,caption})});const d=await res.json();if(d.success){if(!isAdmin) addUsage(ct.id);alert("✅ Publicado en Instagram!");}else{alert("❌ Error: "+(d.error||"No se pudo publicar"));}setPublishing(false);};reader.readAsDataURL(b);}catch(e){alert("❌ Error: "+e.message);setPublishing(false);}}} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",background:publishing?"#666":"linear-gradient(135deg,#833AB4,#E1306C,#F77737)",border:"none",borderRadius:10,color:"#fff",fontSize:12,fontWeight:600,cursor:publishing?"wait":"pointer",opacity:publishing?.6:1}}>📸 {publishing?"Publicando...":"Publicar en Instagram"}</button>}
            </div>}
          </Card>
...
const ClientSettings = ({ user, setUser, onChangePlan }) => {
  ...
  return (
    <Section title="Mi Cuenta" sub="Perfil, notificaciones y plan.">
      ...
      <Card><div style={{ fontSize: 16, fontWeight: 600, color: t.tx, marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}><Ic name="help" size={18}/> Soporte</div><div style={{ fontSize: 14, color: t.txS }}>¿Necesitas ayuda? <a href="mailto:soporte.datagrowth.agency@gmail.com" style={{ color: t.ac, textDecoration: "none" }}>soporte.datagrowth.agency@gmail.com</a></div></Card>
    </Section>
  );
};
