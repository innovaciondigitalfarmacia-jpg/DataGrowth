            ...
            <div style={{ padding: 14, background: t.bgI, borderRadius: 12, border: "1px solid " + t.brd, marginBottom: 14 }}>
              ...
              :
                <button onClick={async () => {
                  ...
                  const handler = async (e) => { if (e.data?.type === "ig_connected") { window.removeEventListener("message", handler); try { const { data: updated } = await supabase.from("brands").select("*").eq("id", f.id).single(); if (updated) { u("ig_token", updated.ig_token || ""); u("ig_user_id", updated.ig_user_id || ""); u("instagram", updated.instagram || "@" + e.data.username); } else { u("ig_user_id", "connected"); u("instagram", "@" + e.data.username); } } catch { u("ig_user_id", "connected"); u("instagram", "@" + e.data.username); } alert("✅ Instagram conectado: @" + e.data.username); } };
